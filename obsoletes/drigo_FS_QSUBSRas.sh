...
$FREESURFER_HOME/bin/recon-all -skullstrip -wsthresh 10 -clean-bm -no-wsgcaatlas -i $PROJECT/$FILE -s ${FILE/$ITYPE} -sd $PROJECT -autorecon-all | tee $QSUBS/tee_${FILE/$ITYPE} " >> $QSUBS/${FILE/$ITYPE}.qsub
